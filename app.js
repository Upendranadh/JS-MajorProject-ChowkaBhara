...
//Function to update player Icons on screen
...
function updateToken(ids, className) {
  ...
  if (state.personTobeRolled === 1 && classNameUpdated[1] === "player1") {
    ...
    if (state.currentdiceValue + curPos <= 15) {
      ...
      // domelements.infoText.textContent = "Please select a valid PLayer";
    ...
    // domelements.infoText.textContent = `Please select Player ${state.personTobeRolled}`;
    alert(`Please select Player ${state.personTobeRolled}`);
  }
}

//Function to remove and move player to next place
...
//function to updateValueInState of player
function updateValueInPlayerState(
  ...
  switch (player) {
    case 1: {
      // domelements.infoText.style.backgroundcolor = "red";
...
function checkWin(player) {
  if (
    ...
    // domelements.infoText === "Player Won the Match";
    console.log(
      player.currentposition[keys[0]] === 24 ||
        player.currentposition[keys[1]] === 24 ||
        player.currentposition[keys[2]] === 24 ||
